...
`get_account`, `search_items`, `get_item`, `get_layer`, `query_layer`, `publish_file`, `publication_status`, `create_web_map` are independently selectable. All tools are selected on initial connection; turning the connector on selects all tools as with other connectors. Unchecked tools and disabled connections are rejected before execution. Read-only pages allow reading; users fork a page before publishing or creating maps. ArcGIS enforces the connected account's privileges.
