...
Each visitor connects their own ArcGIS account. OAuth storage is namespaced by the authenticated AgenticWiKi principal (or its guest identity), and configuration responses are private/no-store. Publication recovery IDs are scoped to both the AgenticWiKi principal and the ArcGIS account. The shared client ID identifies the application only; it never serves as a shared user token. The default portal is www.arcgis.com so users can choose their own organization; the Organization URL field also accepts organization-specific portals. Reading maps only requires access to those maps. Publishing additionally requires `portal:publisher:publishFeatures`. Browser login to ArcGIS alone does not authorize AgenticWiKi. Source reads reuse existing current-user connector/page permissions. No ArcGIS credential is sent to the AgenticWiKi server or put in links/page content.
