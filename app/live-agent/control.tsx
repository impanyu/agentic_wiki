...
export function LiveAgentControl({pageId,disabled}:{pageId:string;disabled?:boolean}){
 ...
 const pre=locale.startsWith('zh')?'':'in ';const until=(ms:number)=>{const m=Math.max(0,Math.round((ms-Date.now())/60000));return m<1?t('in under a minute'):m<60?pre+m+' '+t('min'):m<1440?pre+Math.round(m/60)+' '+t('h'):pre+Math.round(m/1440)+' '+t('d');};
