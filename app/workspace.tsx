...
import {pageStorageProviders} from './storage/page-scope';
...
export default function Workspace({ user, signIn, signOut }: {
  ...
  return <UiContext.Provider value={ui}><div className="browser-shell" lang={locale}>
    ...
    <main className="browser-content" aria-busy={busy}>
      ...
      {visiblePage ? <article {...pageVisualProps(visiblePage)} className={'answer '+(visiblePage.kind==='dynamic'?'dynamic-answer ':'')+'template-'+(visiblePage.labels.templateId||'wiki-v1')} lang={visiblePage.language==='und'?undefined:visiblePage.language} dir={['ar','he','fa','ur','ps','dv','yi'].includes(visiblePage.language)?'rtl':'ltr'}>
        ...
        {!draft&&<div className="context-actions"><div className="context-action-buttons"><button onClick={()=>void createFork()} disabled={busy||saving}><GitFork size={15}/>{t("Fork")}</button>{canWritePage(visiblePage)&&<button disabled={busy||saving} onClick={()=>{setEditing(true);setSaving(true);}}>{t('Edit page')}</button>}<PageShare key={visiblePage.id} page={visiblePage} disabled={busy||saving} onAccess={changeVisibility}/></div><span>{t("Generate a fresh private fork from this question")}</span></div>}
