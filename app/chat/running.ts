...
type Running={message:string;reply:string;startedAt:string};
...
export function trackTurn(pageId:string,userId:string,message:string){
 const k=key(pageId,userId),entry:Running={message,reply:'',startedAt:new Date().toISOString()};turns.set(k,entry);
 return {event(event:unknown){const e=event as {type?:string;text?:unknown};if(e?.type==='reply'&&typeof e.text==='string')entry.reply=e.text;},async finish(){if(turns.get(k)===entry)turns.delete(k);}};
}
