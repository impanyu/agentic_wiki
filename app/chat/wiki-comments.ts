...
export async function postWikiComment(request:Request,page:AnswerPage,viewer:Viewer,data:{message?:string;saveDraftId?:string;mentions?:Mention[]},onStatus?:(message:string)=>void){
 await importLegacyComments(page.id,viewer.userId,viewer.userName);
 const agent=await commentAgent(page.id,viewer);
 ...
 const run=async(onReply?:(text:string)=>void,signal?:AbortSignal)=>{
  ...
  const result=await editWiki(page,data.message!,context,viewer.userId,agent,onReply,signal,attachments.parts,message=>{(activityEmit||onStatus)?.(message);});
