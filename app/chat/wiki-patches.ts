...
export const wikiEditSchema=z.object({apply:z.boolean(),discard:z.boolean(),reply:z.string().min(1).max(12000),title:z.string().min(1).max(200).nullable(),summary:z.string().max(2000).nullable(),indexEntries:z.array(indexEntry).min(2).max(30).nullable().optional(),edits:z.array(z.object({operation:z.enum(['replace','append','prepend']),oldText:z.string(),newText:z.string()}).strict()).max(60)}).strict();
export const wikiEditFormat={type:'object',additionalProperties:false,properties:{apply:{type:'boolean'},discard:{type:'boolean'},reply:{type:'string',minLength:1,maxLength:12000},title:{type:['string','null'],minLength:1,maxLength:200},summary:{type:['string','null'],maxLength:2000},indexEntries:{type:['array','null'],minItems:2,maxItems:30,items:{type:'object',additionalProperties:false,properties:{question:{type:'string',minLength:1,maxLength:500},description:{type:'string',minLength:1,maxLength:1200},group:{type:'string',maxLength:120}},required:['question','description','group']}},edits:{type:'array',maxItems:60,items:{type:'object',additionalProperties:false,properties:{operation:{type:'string',enum:['replace','append','prepend']},oldText:{type:'string'},newText:{type:'string'}},required:['operation','oldText','newText']}}},required:['apply','discard','reply','title','summary','indexEntries','edits']};
export function applyWikiPatches(base:{title:string;summary:string;body:string;indexEntries?:z.infer<typeof indexEntry>[]},decision:z.infer<typeof wikiEditSchema>){
 ...
 return {title:decision.title??base.title,summary,body,indexEntries};
}
