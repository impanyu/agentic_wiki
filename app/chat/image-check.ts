import {parseGoogleMapsUrl} from '@/app/maps/url';
...
export async function unreachableImages(body:string,signal?:AbortSignal){
 // Google Maps links render as embedded maps, not images.
 const remote=[...new Set(imageUrls(body).filter(url=>url.startsWith('https://')&&!parseGoogleMapsUrl(url)))].slice(0,12);
