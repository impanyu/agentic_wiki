...
export async function editWiki(page:AnswerPage,message:string,conversation:unknown,userId:string,agent:Agent,onReply?:(text:string)=>void,signal?:AbortSignal,files:FilePart[]=[]){
 ...
 if(imageOnly&&illustrations.length&&canWritePage(current)){
 ...
 const base=pending||current;
 const task={message,editRequested,canEdit:canWritePage(current),illustrations,conversation,editingBase:{title:base.title,summary:base.summary,body:base.body},page:{sources:current.sources,language:current.language}};
 ...
 const editDraft=canWritePage(current)&&(decision.apply||illustrations.length>0)&&!decision.discard?await stageEdit(agent,current,content):decision.discard?null:pending;
