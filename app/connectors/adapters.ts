...
export function apiOperation(provider:string,name:string,raw:unknown){
 ...
 if(provider==='adma'){
  ...
  if(name==='file_metadata'){const a=parse({file_id:id});path='/api/v1/files/'+a.file_id+'/metadata/';}
  ...
  else if(name==='list_files'){const a=parse({folder_id:id.optional(),is_public:visibility,file_type:z.string().min(1).max(100).optional()});path='/api/v1/files/';query={folder_id:a.folder_id,is_public:a.is_public===undefined?undefined:String(a.is_public),file_type:a.file_type};}
