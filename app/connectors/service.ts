...
export async function createConnection(userId:string,raw:unknown){signedIn(userId);if(!vaultReady())throw Error('Connector encryption is not configured on the server.');const d=connectionInput.parse(raw),preset=d.preset?presetById(d.preset):undefined;
 ...
 if(preset?.auth==='ssh'&&!d.username)throw Error(preset.id==='unl-vpn'?'A TrueYou username (NUID@nebraska.edu) is required.':'An HCC username is required.');
 if(preset?.auth==='password'&&(!d.username||!d.token))throw Error('Enter the account username and password.');
 const credential=preset?.auth==='ssh'?JSON.stringify({username:d.username,sessionId:crypto.randomUUID()}):preset?.auth==='password'?JSON.stringify({username:d.username,password:d.token}):d.token||'';
 ...
 if(kind==='api'){if(!['unl-hcc','unl-vpn','unl-adapt'].includes(preset!.id))await verifyApiConnector(preset!.id,credential);tools=apiTools[preset!.id];}else tools=await discoverMcp(url,d.token);
...
export async function removeConnection(userId:string,id:string){signedIn(userId);if(id==='arcgis'){await updateConnection(userId,id,{enabled:false});await disconnectArcgis(userId);}else if(isStorage(id)){await updateConnection(userId,id,{enabled:false});await disconnectStorage(id as StorageProvider,userId);}else{const c=(await connections(userId)).find(x=>x.id===id);if(c?.provider==='unl-vpn')await (await import('./unl-vpn')).vpnDisconnect(id).catch(()=>{});await database().prepare('DELETE FROM user_connectors WHERE owner_id=? AND id=?').bind(userId,id).run();await vaultDelete(await vaultPath(userId,'connector-'+id));}return {disconnected:true};}
...
async function execute(c:Connection,userId:string,tool:string,args:unknown,signal?:AbortSignal,pageId?:string){
 ...
 const secret=await vaultRead(await vaultPath(userId,'connector-'+c.id));if(!secret)throw Error('Reconnect this connector.');const vpnId=c.provider==='unl-adapt'?(await connections(userId)).find(x=>x.provider==='unl-vpn'&&x.enabled&&x.connected)?.id||null:null;
 const result=c.provider==='unl-vpn'?{connected:await (await import('./unl-vpn')).vpnIsUp(c.id)}:c.provider==='unl-adapt'?await (await import('./adapt')).executeAdaptConnector({secret:secret.token,vpnConnectorId:vpnId},tool,args):c.provider==='unl-hcc'?await (await import('./hcc')).executeHccConnector(secret.token,tool,args,signal):c.provider==='adma'&&(tool==='list_processing_tools'||tool==='processing_status'||tool.startsWith('run_'))?await (await import('@/app/adma/processing-server')).executeProcessing(userId,c.id,secret.token,tool,args,signal):c.kind==='api'?await executeApiConnector(c.provider!,secret.token,tool,args,signal):await callMcp(c.url!,secret.token,tool,args,signal);const json=JSON.stringify(result);return JSON.parse(secret.token?json.split(secret.token).join('[redacted]'):json);
}
