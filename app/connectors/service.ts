...
export async function updateConnection(userId:string,id:string,raw:unknown){signedIn(userId);const d=z.object({enabled:z.boolean().optional(),allowed:z.array(z.string()).max(200).optional(),automatic:z.array(z.string()).max(200).optional(),refresh:z.boolean().optional()}).strict().parse(raw);
 ...
 const names=new Set(tools.map(t=>t.name)),allowed=(d.allowed||c.allowed).filter(n=>names.has(n)),automatic=c.kind==='storage'?['list','read']:(d.automatic||c.automatic).filter(n=>allowed.includes(n)&&(!d.refresh||JSON.stringify(c.tools.find(t=>t.name===n))===JSON.stringify(tools.find(t=>t.name===n))));
...
export async function removeConnection(userId:string,id:string){signedIn(userId);if(isStorage(id)){await updateConnection(userId,id,{enabled:false});await disconnectStorage(id as StorageProvider,userId);}else{await database().prepare('DELETE FROM user_connectors WHERE owner_id=? AND id=?').bind(userId,id).run();await vaultDelete(await vaultPath(userId,'connector-'+id));}return {disconnected:true};}
...
export async function callConnector(userId:string,id:string,tool:string,args:unknown,pageId?:string,signal?:AbortSignal){
 ...
 const shared=pageId&&(await getPage(pageId,userId))?.visibility==='public';
 const automatic=c.automatic.includes(tool)&&!shared;
...
export async function resolveConnectorAction(userId:string,id:string,approve:boolean){signedIn(userId);if(!/^[a-f0-9-]{36}$/.test(id))throw Error('Invalid action.');const lease=await lock('connector-action:'+id,65000);if(!lease)throw Error('Action is running.');try{
 ...
 try{const result=await execute(c,userId,a.tool,saved.args);await vaultWrite(path,{args:saved.args,result});await database().prepare("UPDATE connector_actions SET state='complete' WHERE id=?").bind(id).run();return result;}catch(e){await database().prepare("UPDATE connector_actions SET state='uncertain' WHERE id=?").bind(id).run();throw e;}
