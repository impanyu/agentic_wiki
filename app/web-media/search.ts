...
async function nasa(query:string,kind:PublicMediaKind,signal?:AbortSignal):Promise<PublicMediaResult[]>{
 if(!['all','image','video','audio'].includes(kind))return [];const requested=kind==='all'?'image,video,audio':kind;
 const data=await json('https://images-api.nasa.gov/search?'+new URLSearchParams({q:query,media_type:requested,page_size:'15'}),signal);
 return (data.collection?.items||[]).flatMap((item:any)=>{const meta=item.data?.[0],mediaKind=meta?.media_type;if(!meta||!['image','video','audio'].includes(mediaKind))return [];const preview=item.links?.find((link:any)=>link.rel==='preview')?.href||item.links?.[0]?.href;return [{kind:mediaKind,title:clean(meta.title)||query,source:'https://images.nasa.gov/details-'+encodeURIComponent(meta.nasa_id),url:String(preview||item.href),preview:preview?String(preview):undefined,creator:clean(meta.secondary_creator||meta.center,200)||undefined,license:'NASA media usage guidelines',description:clean(meta.description),provider:'NASA Images'} as PublicMediaResult];});
}
async function libraryOfCongress(query:string,kind:PublicMediaKind,signal?:AbortSignal):Promise<PublicMediaResult[]>{
 const paths:Record<PublicMediaKind,string[]>={all:['photos','film-and-videos','audio-recordings','maps','books'],image:['photos','maps'],video:['film-and-videos'],audio:['audio-recordings'],document:['books','manuscripts','newspapers'],dataset:[]};if(!paths[kind].length)return [];
 const settled=await Promise.allSettled(paths[kind].map(async path=>{const data=await json('https://www.loc.gov/'+path+'/?'+new URLSearchParams({q:query,fo:'json',c:'5'}),signal);return (data.results||[]).flatMap((item:any)=>{const source=String(item.id||item.url||'');if(!source.startsWith('http'))return [];const mediaKind=path==='film-and-videos'?'video':path==='audio-recordings'?'audio':['photos','maps'].includes(path)?'image':'document';const preview=Array.isArray(item.image_url)?item.image_url.at(-1):undefined;return [{kind:mediaKind,title:clean(item.title)||query,source,url:source,preview,creator:clean(Array.isArray(item.contributor)?item.contributor.join(', '):item.contributor,200)||undefined,description:clean(Array.isArray(item.description)?item.description[0]:item.description),provider:'Library of Congress'} as PublicMediaResult];});}));return settled.flatMap(result=>result.status==='fulfilled'?result.value:[]);
}
async function zenodo(query:string,kind:PublicMediaKind,signal?:AbortSignal):Promise<PublicMediaResult[]>{
 const data=await json('https://zenodo.org/api/records?'+new URLSearchParams({q:query,size:'15'}),signal);
 return (data.hits?.hits||[]).flatMap((item:any)=>{const type=String(item.metadata?.resource_type?.type||'publication'),mapped:PublicMediaResult['kind']=type==='dataset'?'dataset':type==='image'?'image':type==='video'?'video':type==='audio'?'audio':'document';if(kind!=='all'&&kind!==mapped)return [];const source=String(item.links?.html||item.doi_url||'');if(!source)return [];return [{kind:mapped,title:clean(item.metadata?.title||item.title)||query,source,url:source,creator:clean((item.metadata?.creators||[]).map((creator:any)=>creator.name).join(', '),200)||undefined,license:clean(item.metadata?.license?.id,100)||undefined,description:clean(item.metadata?.description),provider:'Zenodo'} as PublicMediaResult];});
}
export async function searchPublicMedia(query:string,kind:PublicMediaKind='all',signal?:AbortSignal){
 ...
 const results=await Promise.allSettled([openverse(query,kind,signal),commons(query,kind,signal),archive(query,kind,signal),nasa(query,kind,signal),libraryOfCongress(query,kind,signal),zenodo(query,kind,signal)]);
 ...
 return {query,kind,items,providers:['Openverse','Wikimedia Commons','Internet Archive','NASA Images','Library of Congress','Zenodo'],coverage:'Free public and openly licensed catalogs; not a complete index of the whole web.'};
}
