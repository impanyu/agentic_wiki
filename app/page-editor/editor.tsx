...
export function PageEditor({page,metadataOnly=false,onSave,onClose,onFiles}:{page:AnswerPage;metadataOnly?:boolean;onSave:(page:AnswerPage)=>void;onClose:()=>void;onFiles:()=>void}){
 ...
 async function save(){if(!editor||busy)return;setBusy(true);setError('');try{const r=await fetch('/api/pages/'+page.id+'/content',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({title,summary,...(metadataOnly?{metadataOnly:true}:{document:editor.getJSON()}),base:page.updatedAt||page.createdAt})}),d=await r.json() as any;if(!r.ok)throw Error(d.error);setDirty(false);onSave(metadataOnly?{...page,title:d.page.title,summary:d.page.summary,updatedAt:d.page.updatedAt,checkedAt:d.page.checkedAt}:d.page);}catch(e){setError(e instanceof Error?e.message:'Could not save changes.');}finally{setBusy(false);}}
 ...
 return <section className="page-editor" aria-label={t('Edit page')}>
  ...
  {!metadataOnly&&<><div className="editor-toolbar" role="toolbar" aria-label={t('Formatting')}>
