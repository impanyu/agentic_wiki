...
const [busy,setBusy]=useState(false),[error,setError]=useState('');const zh=page.language.startsWith('zh'),index=page.contextIndex;if(!index)return null;
 ...
 return <section className="context-index"><header><strong>{index.items.length} {t("results")}</strong><button disabled={busy} onClick={()=>void refresh()}>{t("Refresh")}</button></header>{!index.items.length&&<p>{index.kind==='jobs'?(t("No tasks are running right now.")):(t("No matching pages found."))}</p>}<ul>{index.items.map(item=><li key={item.id}>{item.pageId?<button onClick={()=>onOpen(item.pageId!,item.title)}>{item.title}</button>:<strong>{item.title}</strong>}<div><span>{item.kind==='static'?(t("Wiki")):t(item.kind)}</span>{item.state&&<span>{t(item.state)}</span>}<time>{new Date(item.createdAt).toLocaleString(locale)}</time></div>{item.summary&&<p>{item.summary}</p>}</li>)}</ul>{error&&<p role="alert">{t(error)}</p>}</section>;
}
