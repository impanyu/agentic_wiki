...
export const myContextsSql=`SELECT id,title,kind,category,summary,created_at createdAt FROM pages WHERE owner_id=? AND (?='all' OR kind=?) AND NOT EXISTS(SELECT 1 FROM page_aliases a WHERE a.id=pages.id) ORDER BY created_at DESC,id`;
...
export async function executeContextIndex(page:AnswerPage,raw:unknown,userId:string){
 ...
 if(page.dynamic?.indexKind==='jobs'){
  ...
  const rows=await database().prepare(myContextsSql).bind(userId,kind,kind).all<ContextIndexItem&{category:string}>();
