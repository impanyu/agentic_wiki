import {z} from 'zod';
export const resourceSchema=z.object({space:z.enum(['page','google','dropbox','onedrive','adma']),id:z.string().max(2000),kind:z.enum(['file','folder']),name:z.string().min(1).max(250),connectorId:z.string().uuid().optional()}).strict().refine(r=>r.space==='adma'?!!r.connectorId:!r.connectorId,'ADMA requires its connection ID; other sources use the provider account.');
...
export function assertCopyTarget(sources:Resource[],destination:Resource){for(const source of sources){if(source.space===destination.space&&source.connectorId===destination.connectorId&&source.kind==='folder'&&(!source.id||source.id===destination.id||(['page','dropbox'].includes(source.space)&&destination.id.startsWith(source.id.replace(/\/$/,'')+'/'))))throw Error('Choose a destination outside the source folder.');}}
