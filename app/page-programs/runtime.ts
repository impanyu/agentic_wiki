...
export async function runProgramLoop(program:unknown,input:unknown,userId:string,pageId:string,uploads:Parameters<typeof runProgram>[3],options:{verification?:boolean;signal?:AbortSignal}={}):Promise<{view:PageView;proposals:ProgramProposal[];rounds:number;trace:LoopTrace}>{
 ...
 for(let round=0;round<16;round++){
  ...
  try{
   ...
   else if(options.verification&&c.tool==='resources.move')result=skip('moving requires an explicit user action');
   ...
   else if(c.tool==='resources.browse'){if(c.args.folder)result=await browseResources(page.id,userId,c.args.folder,String(c.args.cursor||''));else{const d=await resourceDirectory(page.id,userId);result={roots:d.roots,limits:d.limits};}}// Programs get only the storage roots and copy limits; the workspace tool directory is for the chat picker and made the reply too large.
   else if(c.tool==='resources.copy'){const live=await getPage(page.id,userId);if(!live||!canWritePage(live))throw Error('PAGE_READ_ONLY');if(!input||typeof input!=='object'||!(('message' in input&&input.message)||('submitted' in input&&input.submitted===true)))throw Error('Copy requires an explicit chat or submitted task.');result=await copyResources(page.id,userId,c.args);}
   else if(c.tool==='resources.move'){const live=await getPage(page.id,userId);if(!live||!canWritePage(live))throw Error('PAGE_READ_ONLY');if(!input||typeof input!=='object'||!(('message' in input&&input.message)||('submitted' in input&&input.submitted===true)))throw Error('Move requires an explicit chat or submitted task.');result=await (await import('@/app/resources/move')).moveResources(page.id,userId,c.args);}
