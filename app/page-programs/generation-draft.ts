...
export function validateGenerationDraft(raw:unknown,question:string,context:AgentContext,webSearched:boolean,dataConsulted=false){
 ...
 if(d.kind==='disambiguation'){
  ...
  if(['context_pages','user_jobs'].includes(d.intent.service)&&d.kind!=='index')throw Error('This request lists the user\u2019s own pages or running tasks: use kind="index" (a live, linked, per-visitor list that refreshes), not a static list written into an article or program.');
...
export async function materializeGenerationDraft(d:GenerationDraft,context:AgentContext){
 ...
 if(d.kind==='native'){templateId=d.nativeApp==='map'?'geo-v1':'data-tools-v1';config={...config,template:'native-app-v1',executor:'native-app-v1',nativeApp:d.nativeApp,...(d.nativeApp==='adma-tools'?{inputFields:[{name:'tool',type:'string' as const,description:'ADMA tool slug: seeding-tool, shape-to-json, si-tool, yield-summary, valid-yield-extractor',required:false}]}:{})};}
