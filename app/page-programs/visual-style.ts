import {customStyleSchema,normalizeCustomStyle,contrast,readableInk,styleFonts,type CustomStyle} from './custom-style';
import type {CSSProperties} from 'react';
export const visualThemes=['auto','custom','analytics','research','creative','productivity','nature','minimal','adma','github','linear','notion','slack','airtable','todoist','brave','deepwiki','huggingface','google','dropbox','onedrive'] as const;
...
export const themePalettes:Record<Exclude<VisualTheme,'auto'|'custom'>,Palette>={
...
export function resolveVisualTheme(theme:unknown,question:string,template=''):Exclude<VisualTheme,'auto'|'custom'>{
 if(typeof theme==='string'&&theme!=='auto'&&theme!=='custom'&&visualThemes.includes(theme as VisualTheme))return theme as Exclude<VisualTheme,'auto'|'custom'>;
 const matches=Object.entries(connectorNames).filter(([,pattern])=>pattern.test(question));
 if(matches.length===1)return matches[0][0] as Exclude<VisualTheme,'auto'|'custom'>;
...
export function pageVisualProps(page:{kind?:string;question?:string;title:string;labels:{templateId?:string};dynamic?:{visualTheme?:VisualTheme;visualDesign?:CustomStyle|null}}){
 if(page.kind!=='dynamic')return {};
 if(page.dynamic?.visualTheme==='custom'&&customStyleSchema.safeParse(page.dynamic.visualDesign).success){
  const d=normalizeCustomStyle(page.dynamic.visualDesign),ink=(background:string)=>contrast(d.text,background)>=4.5?d.text:readableInk(background),link=(background:string)=>contrast(d.accent,background)>=4.5?d.accent:ink(background);
  return {'data-app-theme':'custom','data-app-density':d.density,'data-app-header':d.header,'data-app-cards':d.cardLayout,style:{'--app-accent':d.accent,'--app-on-accent':readableInk(d.accent),'--app-tint':d.tint,'--app-canvas':d.canvas,'--app-surface':d.surface,'--app-text':d.text,'--app-muted':d.muted,'--app-canvas-ink':ink(d.canvas),'--app-tint-ink':ink(d.tint),'--app-link':link(d.surface),'--app-canvas-link':link(d.canvas),'--app-tint-link':link(d.tint),'--app-border':d.border,'--app-radius':d.radius+'px','--app-heading-font':styleFonts[d.headingFont],'--app-body-font':styleFonts[d.bodyFont],'--app-spacing':d.spacing+'px','--app-width':d.width+'px','--app-shadow':d.shadow==='flat'?'none':'0 12px 36px #17203318'} as CSSProperties};
 }
...
export const visualStyleInstructions=' Choose visualTheme for the app appearance. If the task primarily uses one connector, prefer its named theme (adma/github/linear/notion/slack/airtable/todoist/brave/deepwiki/huggingface/google/dropbox/onedrive). A mere mention or comparison is not a primary connector. Otherwise prefer an appropriate existing theme: analytics for data dashboards, research for scholarly work, creative for creative tools, productivity for planning, nature for agricultural/environmental tools, or minimal. Use a preset only when it fits; do not force a distinctive requested design into these broad categories. If no preset suits the task, connector or requested visual identity, select custom and author visualDesign: original hex colors for accent/tint/canvas/surface/text/muted/border; heading and body font families; corner radius, spacing, content width, header treatment, density, shadow and card layout. Light and dark designs are supported. Choose readable contrasting text and distinct interactive states. For a preset or auto, visualDesign must be null. For custom it must be a complete design. Honor an explicit requested style over connector branding. auto delegates to a conservative fallback. These are visual inspirations, not official provider affiliation; never add provider login forms or claim the page is that service.';
