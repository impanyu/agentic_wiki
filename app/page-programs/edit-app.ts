...
const revision=(page:AnswerPage)=>JSON.stringify([page.title,page.summary,page.body,page.dynamic?.template,page.dynamic?.components,page.dynamic?.inputFields,page.dynamic?.sessionInstructions,page.dynamic?.visualTheme,page.labels.templateId]);
...
export async function discussAppEdit(page:AnswerPage,message:string,context:unknown,agent:Agent,files:FilePart[]=[],onReply?:(text:string)=>void,signal?:AbortSignal){
 ...
 if(decision.changeType==='appearance'){config={...page.dynamic,visualTheme:decision.visualTheme};body=decision.instructions;}
 ...
 if(decision.changeType!=='appearance')config={...config,visualTheme:page.dynamic.visualTheme};
...
export async function saveAppDraft(agent:Agent,pageId:string,draftId:string){
 ...
 try{const page=await getPage(pageId,agent.ownerId),draft=await stored(agent,pageId);if(!page||!canWritePage(page)||!page.dynamic||!draft||draft.id!==draftId)throw Error('This app proposal is unavailable.');if(draft.saved)return {page,alreadySaved:true};if(revision(page)!==draft.base)throw Error('The app changed. Ask the agent to revise its proposal.');
