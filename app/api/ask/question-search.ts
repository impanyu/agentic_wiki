import {ensureRoutingScopes} from '@/db/routing-tables';
...
export async function matchQuestion(question:string,vector:number[],language:string,userId:string,agent:Agent,signal?:AbortSignal){
 ...
 while(true){
  const rows=await database().prepare(`SELECT q.id,q.question,q.normalized,q.embedding,q.capability,p.language,q.page_id,p.question canonical_question,json_extract(p.labels,'$.templateId') template_id,p.labels page_labels FROM questions q JOIN pages p ON p.id=q.page_id WHERE NOT (q.normalized LIKE 'url:%' AND q.question=substr(q.normalized,5)) AND p.language=? AND q.id>? AND (p.visibility='public' OR p.owner_id=?) AND (q.routing_scope<>'session' OR p.owner_id=? OR EXISTS(SELECT 1 FROM session_routes r WHERE r.owner_id=? AND r.question_id=q.id AND r.page_id=p.id)) AND NOT EXISTS(SELECT 1 FROM page_aliases a WHERE a.id=p.id) AND NOT EXISTS(SELECT 1 FROM page_replacements r WHERE r.source_id=p.id AND r.user_id=?) ORDER BY q.id LIMIT 100`).bind(language,after,userId,userId,userId,userId).all<Candidate&{page_id:string;embedding:string;normalized:string;canonical_question:string;template_id:string;page_labels:string}>();
