...
export async function POST(request:Request){
 ...
 try{
  ...
  await database().prepare(`DELETE FROM internal_links WHERE source_id=? AND segments=? AND quote=? AND id<>? AND target_id IN (SELECT id FROM pages WHERE visibility='public' OR owner_id=?)`).bind(source.id,segments,data.quote,id,uid).run();
