...
const thumbnail=(body:string)=>body.match(/!\[[^\]]*\]\((https?:\/\/[^\s)]+)/)?.[1]||null;
...
// Home page data: the reader's recently visited pages and the newest public pages.
export async function GET(request:Request){
 const actor=await getActor(request);
 try{
  const visited=await database().prepare(`SELECT p.id,p.title,p.summary,p.kind,p.category,p.labels,substr(p.body,1,4000) body,p.owner_id,max(v.visited_at) at,p.created_at,p.updated_at FROM page_visits v JOIN pages p ON p.id=COALESCE((SELECT page_id FROM page_aliases WHERE id=v.page_id),v.page_id) WHERE v.owner_key=? AND v.visited_at IS NOT NULL AND (p.visibility='public' OR p.owner_id=?) AND p.kind IN ('static','dynamic') GROUP BY p.id ORDER BY at DESC LIMIT 12`).bind(actor.historyKey,actor.userId).all<PageRow>();
  const published=await database().prepare(`SELECT p.id,p.title,p.summary,p.kind,p.category,p.labels,substr(p.body,1,4000) body,p.owner_id,NULL at,p.created_at,p.updated_at FROM pages p WHERE p.visibility='public' AND p.owner_id NOT LIKE 'system:%' AND p.kind IN ('static','dynamic') ORDER BY COALESCE(p.updated_at,p.created_at) DESC LIMIT 12`).all<PageRow>();
  return actor.finish(reply({visited:visited.results.map(row=>card(row,actor.userId)),published:published.results.map(row=>card(row,actor.userId))}));
