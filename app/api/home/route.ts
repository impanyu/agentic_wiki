...
export async function GET(request:Request){
 ...
 try{
  ...
  const published=await database().prepare(`SELECT p.id,p.title,p.summary,p.kind,p.category,p.labels,CASE WHEN instr(p.body,'![')>0 THEN substr(p.body,instr(p.body,'!['),1200) ELSE '' END body,p.owner_id,NULL at,p.created_at,p.updated_at FROM pages p WHERE p.visibility='public' AND p.owner_id NOT LIKE 'system:%' AND p.kind IN ('static','dynamic') ORDER BY COALESCE(p.updated_at,p.created_at) DESC LIMIT 64`).all<PageRow>();
