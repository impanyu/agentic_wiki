...
export async function PUT(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 try{
  ...
  const document=validateDocument(data.document),body=documentMarkdown(document);
  lease=await lock('refresh:'+page.id,90000);if(!lease)return respond({error:'This page is being updated. Please retry.'},409);
  const now=new Date().toISOString(),labels={...page.labels,richContent:document,richBody:body};delete labels.sourceMedia;
