...
import {Clock3,Globe2,LoaderCircle,AppWindow,FileText} from 'lucide-react';
...
export function HomePage({onOpen,revision=0}:{onOpen:(card:HomeCard)=>void;revision?:number}){
 ...
 const [data,setData]=useState<{visited:HomeCard[];published:HomeCard[]}|null>(null),[error,setError]=useState('');
 ...
 const section=(title:string,Icon:typeof Clock3,cards:HomeCard[]|undefined,empty:string)=><section className="home-section" aria-label={title}>
  ...
  {!!cards?.length&&<div className="home-grid">{cards.map(card=><button type="button" className={'home-card'+(card.image?' has-image':'')} key={card.id} onClick={()=>onOpen(card)}>
 ...
 return <div className="home-page">
  ...
  {section(t('Recently published'),Globe2,data?.published,t('No public pages yet.'))}
 </div>;
}
