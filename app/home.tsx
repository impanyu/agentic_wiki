...
import {Clock3,Globe2,LoaderCircle,AppWindow,FileText,X} from 'lucide-react';
...
export function HomePage({onOpen,revision=0}:{onOpen:(card:HomeCard)=>void;revision?:number}){
 ...
 const [data,setData]=useState<{visited:HomeCard[];published:HomeCard[]}|null>(null),[error,setError]=useState(''),[removing,setRemoving]=useState<string|null>(null);
 // Visited cards forget the page's visits; a published card of one's own page unpublishes it.
 async function remove(card:HomeCard,list:'visited'|'published'){setRemoving(list+card.id);setError('');try{const r=list==='visited'?await fetch('/api/history',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({pageId:card.id})}):await fetch('/api/pages/'+encodeURIComponent(card.id),{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({access:'private'})});const d=await r.json() as {error?:string};if(!r.ok)throw Error(d.error||'Could not remove this page.');setData(current=>current?{...current,[list]:current[list].filter(c=>c.id!==card.id)}:current);}catch(e){setError(e instanceof Error?e.message:'Could not remove this page.');}finally{setRemoving(null);}}
 useEffect(()=>{const controller=new AbortController();setError('');fetch('/api/home',{cache:'no-store',signal:controller.signal}).then(async r=>{const d=await r.json() as {visited:HomeCard[];published:HomeCard[];error?:string};if(!r.ok)throw Error(d.error||'Could not load the home page.');setData(d);}).catch(e=>{if(!controller.signal.aborted)setError(e instanceof Error?e.message:'Could not load the home page.');});return()=>controller.abort();},[revision]);
 const when=(at:number|null)=>at?new Date(at).toLocaleDateString(locale,{month:'short',day:'numeric'}):'';
 const section=(list:'visited'|'published',title:string,Icon:typeof Clock3,cards:HomeCard[]|undefined,empty:string)=><section className="home-section" aria-label={title}>
  ...
  {!!cards?.length&&<div className="home-grid">{cards.map(card=><div className="home-card-shell" key={card.id}>{(list==='visited'||card.owned)&&<button type="button" className="home-card-remove" disabled={removing!==null} aria-label={t(list==='visited'?'Remove from history':'Unpublish')+': '+card.title} title={t(list==='visited'?'Remove from history':'Unpublish (make private)')} onClick={()=>void remove(card,list)}>{removing===list+card.id?<LoaderCircle className="spinner" size={13}/>:<X size={13}/>}</button>}<button type="button" className={'home-card'+(card.image?' has-image':'')} onClick={()=>onOpen(card)}>
 ...
 return <div className="home-page">
  ...
  {section('visited',t('Recently visited'),Clock3,data?.visited,t('No visited pages yet.'))}
  {section('published',t('Recently published'),Globe2,data?.published,t('No public pages yet.'))}
 </div>;
}
