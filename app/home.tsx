...
export function HomePage({onOpen,revision=0}:{onOpen:(card:HomeCard)=>void;revision?:number}){
 ...
 // While a page is generating, refresh the first page so finished pages replace their placeholder.
 const generating=!!data?.visited.some(c=>c.generating);
 useEffect(()=>{if(!generating)return;const refresh=()=>{if(document.hidden)return;fetch('/api/home',{cache:'no-store'}).then(r=>r.json() as Promise<{visited?:HomeCard[];next?:number|null}>).then(d=>{if(!d.visited)return;setData(current=>{if(!current)return current;const fresh=d.visited!,ids=new Set(fresh.map(c=>c.id));return {visited:[...fresh,...current.visited.filter(c=>!ids.has(c.id)&&!c.generating)],next:current.next};});}).catch(()=>{});};const timer=setInterval(refresh,5000);document.addEventListener('visibilitychange',refresh);return()=>{clearInterval(timer);document.removeEventListener('visibilitychange',refresh);};},[generating]);
 ...
 const section=(title:string,Icon:typeof Clock3,cards:HomeCard[]|undefined,empty:string)=><section className="home-section" aria-label={title}>
  ...
  {!!cards?.length&&<div className="home-grid">{cards.map(card=><div className="home-card-shell" key={card.id}>{!card.generating&&<button type="button" className="home-card-remove" disabled={removing!==null} aria-label={t('Remove from history')+': '+card.title} title={t('Remove from history')} onClick={()=>void remove(card)}>{removing===card.id?<LoaderCircle className="spinner" size={13}/>:<X size={13}/>}</button>}<button type="button" className={'home-card'+(card.image?' has-image':'')} onClick={()=>onOpen(card)}>
   ...
   <span className="home-card-body"><strong>{card.title}</strong><small>{card.summary}</small><span className="home-card-meta"><em>{t(card.generating?'Generating':card.kind==='dynamic'?'Web app':'Wiki page')}</em>{card.category&&<span>{card.category}</span>}{when(card.at)&&<span>{when(card.at)}</span>}</span>{(card.liveAgent||card.agentRunning)&&<span className="home-card-agents">{card.generating?<span className="home-agent running" title={t('This page is still being generated')}><LoaderCircle className="spinner" size={11}/>{t('Generating…')}</span>:card.agentRunning&&<span className="home-agent running" title={t('An agent is working on this page now')}><LoaderCircle className="spinner" size={11}/>{t('Agent running')}</span>}{card.liveAgent&&<span className="home-agent live" title={t('Under live agent maintenance')}><Bot size={11}/>{t('Live agent on')}</span>}</span>}</span>
  </button></div>)}</div>}
