...
export function HomePage({onOpen,revision=0}:{onOpen:(card:HomeCard)=>void;revision?:number}){
 ...
 useEffect(()=>{if(!generating)return;const timer=setInterval(()=>{if(document.hidden)return;fetch('/api/home',{cache:'no-store'}).then(r=>r.json() as Promise<{visited?:HomeCard[];next?:number|null}>).then(d=>{if(!d.visited)return;setData(current=>{if(!current)return current;const fresh=d.visited!,ids=new Set(fresh.map(c=>c.id));return {visited:[...fresh,...current.visited.filter(c=>!ids.has(c.id)&&!c.generating)],next:current.next};});}).catch(()=>{});},5000);return()=>clearInterval(timer);},[generating]);
