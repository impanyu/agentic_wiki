...
export function sandboxDocument(app:SandboxDefinition){
 ...
 const bootstrap=`window.addEventListener('error',()=>{const p=document.createElement('p');p.textContent='This app encountered an error. Please try the question again.';p.setAttribute('role','alert');document.body.appendChild(p);});`;
