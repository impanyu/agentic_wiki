...
export function PageAgentChat({page,onResult,onOpenQuestion}:{page:AnswerPage;onOpenQuestion?:(question:string)=>void;onResult:(page:AnswerPage)=>void}){
 ...
 async function send(e:React.FormEvent){
  e.preventDefault();if(!message.trim()||busy)return;setBusy(true);setError('');const controller=new AbortController();request.current=controller;const text=message.trim(),sentReferences=references;setReferences([]);setMessage('');setPending({user:text,reply:'',authorName:userName,createdAt:new Date().toISOString()});
  try{
