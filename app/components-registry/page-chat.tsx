...
export function PageAgentChat({page,onResult}:{page:AnswerPage;onResult:(page:AnswerPage)=>void}){
 ...
 return <section className="page-agent page-chat" aria-label={t("Chat")}>
  ...
  {editDraft&&canWritePage(page)&&<div className="edit-proposal"><h3>{t("Proposed changes · Not saved")}</h3><details><summary>{wiki?t("Preview revised article"):t("Preview app changes")}</summary><div className="edit-proposal-preview"><h4>{editDraft.title}</h4><p>{editDraft.summary}</p><ChatMarkdown text={editDraft.body}/></div></details><button disabled={busy} onClick={()=>void save()}>{busy?'…':t("Save changes")}</button></div>}
  <form onSubmit={send} className="chat-composer"><label className="sr-only" htmlFor={'chat-'+page.id}>{t("Message AgenticWiKi")}</label><textarea id={'chat-'+page.id} aria-label={t("Message AgenticWiKi")} placeholder={t("Message AgenticWiKi…")} value={message} onChange={e=>setMessage(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.nativeEvent.isComposing){e.preventDefault();e.currentTarget.form?.requestSubmit();}}} maxLength={2000} rows={2} disabled={busy}/><div className="chat-composer-actions"><span>{t("Shift + Enter for a new line")}</span><button aria-label={t("Send message")} disabled={!ready||busy||!message.trim()}><ArrowUp size={20}/></button></div></form>
