...
import {VoiceInput,type VoiceHandle} from '@/app/voice-input';
import {useEffect,useState,useRef} from 'react';
import {ArrowUp,MessageSquare,LoaderCircle} from 'lucide-react';
...
export function PageAgentChat({page,onResult,onOpenQuestion}:{page:AnswerPage;onOpenQuestion?:(question:string,passage:string)=>void;onResult:(page:AnswerPage)=>void}){
 ...
 const voice=useRef<VoiceHandle|null>(null),[recording,setRecording]=useState(false),[voiceSending,setVoiceSending]=useState(false);
 // Sending while recording stops the recording and sends typed text plus the transcript.
 async function send(e:React.FormEvent){
  e.preventDefault();if(uploading||voiceSending)return;
  let text=message.trim();
  if(recording&&voice.current){setVoiceSending(true);try{const spoken=(await voice.current.finish()).trim();text=[text,spoken].filter(Boolean).join(' ').slice(0,2000);}finally{setVoiceSending(false);}}
  if(!text)return;const item={id:crypto.randomUUID(),text,references};setReferences([]);setMessage('');
