...
export function PageAgentChat({page,onResult}:{page:AnswerPage;onResult:(page:AnswerPage)=>void}){
 ...
 return <section className="page-agent page-chat" aria-label={zh?'对话':'Chat'}>
  ...
  {editDraft&&page.owned&&<div className="edit-proposal"><h3>{zh?'建议的更改 · 尚未保存':'Proposed changes · Not saved'}</h3><details><summary>{zh?'预览修改后的文章':(wiki?'Preview revised article':'Preview app changes')}</summary><div className="edit-proposal-preview"><h4>{editDraft.title}</h4><p>{editDraft.summary}</p>{editDraft.body.split(/\n\n+/).map((p,i)=>{const image=/^!\[([^\]]*)\]\((https:\/\/(?:upload|thumb)\.wikimedia\.org\/[^\s)]+)\)/.exec(p);return image?<figure key={i}><img src={image[2]} alt={image[1]} style={{maxWidth:'100%',maxHeight:360,objectFit:'contain'}}/><figcaption>{image[1]}</figcaption></figure>:<p key={i}>{p}</p>;})}</div></details><button disabled={busy} onClick={()=>void save()}>{busy?'…':zh?'保存更改':'Save changes'}</button></div>}
