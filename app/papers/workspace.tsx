...
export function PaperWorkspace({pageId,sources,revision=0}:{pageId:string;sources:PaperSource[];revision?:number}){
 ...
 useEffect(()=>{setSelected(remotePdf?'source':'');setUrl(old=>{if(old.startsWith('blob:'))URL.revokeObjectURL(old);return remotePdf;});setError('');setReader(true);},[pageId,remotePdf]);
 ...
 return <section className={'paper-workspace '+(reader?'paper-reader-open':'paper-reader-closed')}>
  ...
  {reader&&<div className="paper-document" aria-label={t('Paper document')}>{url?<iframe title={selected==='source'?t('Original paper PDF'):files.find(f=>f.id===selected)?.name||t('Paper PDF')} src={url}/>:<div className="paper-empty"><FileText size={38}/><strong>{t('Open the original paper')}</strong><p>{t('Upload the PDF to Page files, then choose it here. The page agent can read selected PDF pages and update this wiki after you approve its proposal.')}</p></div>}{busy&&<p role="status">{t('Loading…')}</p>}{error&&<p role="alert">{t(error)}</p>}</div>}
 </section>;
}
