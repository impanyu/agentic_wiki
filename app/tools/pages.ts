...
export function nativePageAddress(id:string,query:NativeQuery){
 ...
 for(const [key,target,max] of [['page','sourcePage',200],['file','file',200],['connector','connector',200],['name','name',300],['space','space',20],['tool','tool',60]] as const){const value=query[key];if(typeof value==='string'&&value.length<=max)parameters[target]=value;}
