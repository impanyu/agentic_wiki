...
export async function generateImage(pageId:string,ctx:AgentContext,args:{prompt:string;name:string;size?:string;quality?:string},signal?:AbortSignal){
 ...
 const stored=await storePageFile(pageId,ctx.userId,imageName(args.name,'png'),decode(data.data[0].b64_json),ctx.language||'en','',ctx.pageId?undefined:ctx.pendingFiles);
