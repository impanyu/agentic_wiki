...
export async function runAgentResponse(agent:Agent,payload:Record<string,any>,signal?:AbortSignal,options:AgentOptions={},request?:(payload:Record<string,any>)=>Promise<any>){
 ...
 return runToolLoop({payload,signal,maxRounds:options.maxRounds,maxCalls:options.maxCalls,maxInputChars:options.maxInputChars,maxValidationRetries:options.maxValidationRetries,validateFinal:options.validateFinal,event:async e=>{await journal(e);options.onEvent?.(e);},request:request||((current)=>api('responses',current,signal)),execute:async(name,args)=>{
