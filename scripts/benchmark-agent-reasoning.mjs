...
const request=async body=>{const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:'Bearer '+process.env.OPENAI_API_KEY,'Content-Type':'application/json'},body:JSON.stringify(body),signal:AbortSignal.timeout(90000)});if(!r.ok)throw Error('OpenAI HTTP '+r.status);return r.json();};
...
for(const effort of ['low','medium']){
 ...
 const began=Date.now();const r=await request({model:'gpt-5.6-terra',store:false,reasoning:{effort},instructions:'Return the minimal JSON expression program for the requested reusable calculator. Expressions are number literals, {input:field}, or {op:"add"|"subtract"|"multiply"|"divide",args:[expressions]}. Output plain JSON only.',input:'A percentage change calculator: inputs before and after; output percent = (after-before)/before * 100. Name the output percent. Return {kind:"expression-program",outputs:{percent:expression}}.',text:{format:{type:'json_object'}},max_output_tokens:3000});
 const p=JSON.parse(text(r));assert.equal(p.kind,'expression-program');
 ...
 console.log(JSON.stringify({task:'expression_generation',effort,passed:true,durationMs:Date.now()-began,usage:r.usage}));
}
