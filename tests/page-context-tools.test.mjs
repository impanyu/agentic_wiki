...
const strip=p=>readFileSync(p,'utf8').replace(/^import .*;$/gm,'');const load=async s=>import('data:text/javascript;base64,'+Buffer.from(ts.transpile(readFileSync('app/page-permissions.ts','utf8').replace(/import [\s\S]*?from ['"][^'"]+['"];?/g,'')+'\n'+s,{module:ts.ModuleKind.ESNext,target:ts.ScriptTarget.ES2022})).toString('base64'));
...
test('dedicated agent executes context tool calls before producing its final reply',async()=>{
 ...
 globalThis.agentLoopTest={getPage:async()=>({owned:false,visibility:'public'}),database:()=>({prepare:()=>({bind:()=>({run:async()=>({}),all:async()=>({results:[]})})}),batch:async()=>[]}),model:()=> 'test',partialReply:()=>'',output:r=>r.output.filter(o=>o.type==='message').flatMap(o=>o.content).map(c=>c.text).join(''),api:async(path,payload)=>{if(++calls===1)return {output:[{type:'function_call',name:'read_page_context',arguments:JSON.stringify({resource:'history',query:'earlier',before:null,fileId:null}),call_id:'call1'}]};assert.ok(payload.input.some(i=>i.type==='function_call_output'&&i.output.includes('earlier detail')));return {output:[{type:'message',content:[{type:'output_text',text:'{"reply":"Found earlier detail"}'}]}]};},streamArticle:async()=>{},scoped:{pageContextTool:{},pageTaskTool:{},readPageContext:async()=>{read=true;return {data:'earlier detail'}}}};
 const code=('const connectorAgentCall=async()=>null;const connectorTools=[],connectorInstructions="";\n'+(runtimeSource+strip('app/components-registry/agents.ts'))).replace("await import('@/app/chat/context-tools')",'globalThis.agentLoopTest.scoped');
 const {askAgent}=await load('const {getPage,database,model,partialReply,output,api,streamArticle}=globalThis.agentLoopTest;\n'+code);
