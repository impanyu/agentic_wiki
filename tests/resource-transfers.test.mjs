...
test('folder copies preserve paths, enforce checked tools and never repeat completed writes',async()=>{
 ...
 writable=false;await assert.rejects(copyResources('p','u',{...req,operationId:crypto.randomUUID()}),/read only/);writable=true;allowed=['list'];await assert.rejects(copyResources('p','u',{...req,operationId:crypto.randomUUID()}),/Enable upload/);
