...
test('only the generator decides ambiguity after page reuse misses',()=>{const source=readFileSync('app/api/ask/route.ts','utf8');assert.doesNotMatch(source,/assessAmbiguity|needsDisambiguation/);const generation=readFileSync('app/page-programs/generate-context.ts','utf8');assert.match(generation,/analyzeGenerationIntent\(brief.question,context.language,generator,signal,context.sourceDocument\)/);});
