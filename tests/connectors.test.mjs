...
test('MCP initializes, sends session headers, follows pagination and closes its session',async()=>{
 ...
 const m=await load('const {toolSchema,remoteRequest}=globalThis.mcpTest;\n'+strip('app/connectors/mcp.ts'));assert.equal((await m.discoverMcp('https://example.org/mcp')).length,2);assert.deepEqual(methods,['initialize','notifications/initialized','tools/list','tools/list','DELETE']);delete globalThis.mcpTest;
});
test('generation and in-page agents both dispatch connector calls as the current user',async()=>{
 let calls=0,owner,tool;
 globalThis.connectorLoop={connectorTools:[{type:'function',name:'list_connectors'},{type:'function',name:'call_connector'}],connectorInstructions:'Use enabled tools.',connectorAgentCall:async(u,n)=>{owner=u;tool=n;return n==='list_connectors'?{connectors:[{id:'adma-connection',name:'ADMA',tools:[{name:'list_files'}]},{id:'remote',name:'Other enabled service',tools:[{name:'read'}]}]}:{found:'authorized result'};},getPage:async()=>({owned:false,visibility:'public'}),canWritePage:()=>false,database:()=>({prepare:()=>({bind:()=>({run:async()=>({}),all:async()=>({results:[]})})}),batch:async()=>[]}),model:()=> 'test',partialReply:()=>'',output:r=>r.output.filter(o=>o.type==='message').flatMap(o=>o.content).map(c=>c.text).join(''),api:async(path,p)=>{assert.ok(p.tools.some(t=>t.name==='call_connector'));assert.ok(p.instructions.includes('adma-connection'));assert.ok(p.instructions.includes('Other enabled service'));if(++calls%2===1)return {output:[{type:'function_call',name:'call_connector',call_id:'c',arguments:'{"connectorId":"remote","tool":"read","argumentsJson":"{}"}'}]};assert.ok(p.input.some(i=>i.type==='function_call_output'&&i.output.includes('authorized result')));return {output:[{type:'message',content:[{type:'output_text',text:'{"reply":"done"}'}]}]};},streamArticle:async()=>{},scoped:{pageContextTool:{},pageTaskTool:{}}};
 const source=(runtimeSource+strip('app/components-registry/agents.ts')).replace("await import('@/app/chat/context-tools')",'globalThis.connectorLoop.scoped');
 const m=await load('const {'+Object.keys(globalThis.connectorLoop).join(',')+'}=globalThis.connectorLoop;\n'+source);
 for(const role of ['content-generation','page:p']){const result=await m.askAgent({id:role,role,ownerId:'viewer'},'answer',{},{});assert.equal(result.reply,'done');assert.equal(owner,'viewer');assert.equal(tool,'call_connector');}assert.equal(calls,4);delete globalThis.connectorLoop;
});
