...
test('share control uses translated captions while keeping permission values unchanged',async()=>{
 ...
 globalThis.uiShareTest={React,useState:value=>[typeof value==='boolean'?true:value,()=>{}],useUi:()=>({locale:'zh-Hans',t:text=>translateUi(text,catalog,zh)}),Copy:()=>null,Share2:()=>null,pageAccess:()=> 'private'};
 const code=readFileSync('app/page-share.tsx','utf8').replace(/^import .*;$/gm,'');
 const source=ts.transpile('const {React,useState,useUi,Copy,Share2,pageAccess}=globalThis.uiShareTest;\n'+code,{module:ts.ModuleKind.ESNext,target:ts.ScriptTarget.ES2022,jsx:ts.JsxEmit.React});
