...
export type Row={id:string;kind:'static'|'dynamic';dynamic_config:string|null;language:string;labels:string;owner_id:string;question:string;title:string;summary:string;body:string;category:string;sources:string;visibility:'public'|'private';created_at:string;updated_at:string|null;checked_at:string|null;question_count:number};
export async function getPage(id:string,userId:string):Promise<AnswerPage|null>{
 ...
 return hydrateComponents({forks:forks.results.length?forks.results:[{id:row.id,title:row.title,visibility:row.visibility,createdAt:row.created_at,isOriginal:true,removable:row.owner_id===userId}],links:links.results.map(link=>({...link,segments:JSON.parse(link.segments),parameters:JSON.parse(link.parameters)})),kind:row.kind,dynamic:row.dynamic_config?JSON.parse(row.dynamic_config):undefined,id:row.id,question:row.question,language:row.language,labels:JSON.parse(row.labels),title:row.title,summary:row.summary,body:row.body,category:row.category,sources:JSON.parse(row.sources),visibility:row.visibility,owned:row.owner_id===userId,createdAt:row.created_at,updatedAt:row.updated_at||undefined,checkedAt:row.checked_at||undefined,questionCount:row.question_count},userId);
}
