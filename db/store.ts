...
// The app runs as one server process, so a lock this process never issued was
// left by a process that exited (a restart or deploy) and can be taken over.
const heldLocks=new Set<string>();
export async function lock(name:string,ttl=240000){const token=crypto.randomUUID();const now=Date.now();let result=await database().prepare(`INSERT INTO generation_locks(name,token,expires) VALUES(?,?,?) ON CONFLICT(name) DO UPDATE SET token=excluded.token,expires=excluded.expires WHERE generation_locks.expires < ?`).bind(name,token,now+ttl,now).run();
 if(!result.meta.changes&&process.env.AGENTICWIKI_MULTI_PROCESS!=='true'){const current=await database().prepare('SELECT token FROM generation_locks WHERE name=?').bind(name).first<{token:string}>();if(current&&!heldLocks.has(current.token))result=await database().prepare('UPDATE generation_locks SET token=?,expires=? WHERE name=? AND token=?').bind(token,now+ttl,name,current.token).run();}
 if(!result.meta.changes)return null;heldLocks.add(token);return token;}
export async function unlock(token:string){heldLocks.delete(token);await database().prepare(`DELETE FROM generation_locks WHERE token=?`).bind(token).run();}
